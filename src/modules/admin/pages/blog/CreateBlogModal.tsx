...
const CreateBlogModal = ({ onRefreshPage }: CreateBlogTypes, ref: any) => {
  ...
  const { isLoading: isLoadingGetUsers, refetch } = useQuery(
    ["user-list"],
    USERS_API.GET_LIST,
    {
      enabled: false,
  ...
  const { isLoading: isLoadingCreateBlog, mutate } = useMutation(
    BLOG_API.CREATE_NEW_BLOG,
    {
      onSuccess: () => {
        messageApi.open({
          type: "success",
  ...
  return (
    <BaseModal
      ...
      closeIcon
      title={
        <BaseTypography className="text-xl">Create a new blog</BaseTypography>
      }
    >
      {contextHolder}
      <BaseForm
        ...
        <BaseRow gutter={[20, 20]}>
          ...
          <BaseCol span={24}>
            <BaseForm.Item name="blogPhoto" label="Photo">
              <BaseInput
                placeholder="Enter your link image of the blog"
                required
              />
            </BaseForm.Item>
          </BaseCol>
          ...
          <BaseCol span={24}>
            ...
            >
              <BaseInput
                placeholder="Enter the tag name of the blog"
                required
              />
            </BaseForm.Item>
          </BaseCol>
          ...
          <BaseCol span={24}>
            ...
            >
              <Select
                placeholder="Choose the user that make the blog"
                options={userSelect}
              />
            </BaseForm.Item>
          </BaseCol>
